...
The homepage scene card uses the existing Unity captures for all ten demos and links each slide to its walkthrough. It advances every five seconds, pauses on mouse hover or keyboard focus, and stops while off screen or in a hidden tab. Previous/next, arrow keys, Home/End, touch swipes, and an explicit Play/Pause control provide manual navigation. Reduced-motion visitors start with rotation paused and see no slide animation. With JavaScript disabled, the first demo and the full demo gallery remain usable. Only the current and next image are loaded eagerly. See `tests/carousel.spec.ts` and [the carousel review](validation/CAROUSEL.md).
